...
impl<'a> Monomorphiser<'a> {
    pub fn monomorphise(items: &Vec<Item>) -> Vec<FunDef> {
        ...
        //Vist all orginal functions to monomorphise function calls
        ...
        while !monomorphiser.generated_funs.is_empty() {
            ...
            let mono_funs_with_name =
                match mono_funs.get_mut(&fun_name) {
    ...
    fn monomorphise_fun_app(&mut self, fun_kind: &mut Option<FunctionKind>, fun: &mut Box<Expr>,
        ...
        fn set_gl_fun_name(fun: &mut Box<Expr>, fun_kind: &mut Option<FunctionKind>, new_fun_name: String) {
        ...
        let (mut fun_name, mut fun_def, mut global_fun_name) = {
            ...
            let (_, fun_def, global_fun_name) =
                match self.funs.iter().find(|(name, _, _)|
                    *name == fun_name) {
                    Some(res) => res,
        ...
        //If the constraint generic args does not contain any identifier
        //TODO i think con_generics is wrong here
    ...
    fn replace_trait_fun_app(&mut self, fun_name: &mut FunctionName,
