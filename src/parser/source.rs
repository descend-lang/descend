...
impl SourceFile {
    ...
    pub fn parse_global_fun_def(&self) -> Result<GlobalFunDef, ParseError<LineCol>> {
        parse_global_fun_def(&self.src)
    }
}
...
mod tests {
    ...
    fn empty() {
        ...
        assert_eq!(src_file, SourceFile {
            src,
            line_offsets: vec![0]
        }, "cannot handle empty source file");
        assert_eq!(src_file.get_line_col(0), Some((1,1)), "cannot get line and column in empty source file");
    }
    ...
    fn one_line() {
        ...
        assert_eq!(src_file.get_line_col(4), Some((1,5)), "cannot get line and column in one line source file");
        assert_eq!(src_file.get_line_col(100), None, "cannot get line and column in one line source file");
    }
