...
pub fn ownership_safe(
    ...
    if p.is_place() {
        ...
        match ty_ctx.place_ty(&most_spec_pl)? {
            Ty::Borrow(BorrowTy::Ref(Provenance::Value(prv_val_name), ref_own, _, _)) => {
            ...
            Ty::Borrow(BorrowTy::Ref(Provenance::Ident(_), ref_own, _, _)) => {
...
fn exists_place_with_ref_to_prv_all_in_reborrow(
    ...
    let at_least_one = all_places.iter().any(|(_, ty)| {
        if let Ty::Borrow(BorrowTy::Ref(Provenance::Value(pn), _, _, _)) = ty {
            prv_name == pn
        } else {
            false
        }
