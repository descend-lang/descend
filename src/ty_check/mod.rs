...
// --------------
//      ⊢ Σ
pub fn ty_check<'a>(compil_unit: &mut CompilUnit, arena: &'a Bump) -> Result<(), ErrorReported> {
    ...
    if let Some(mut main_fun) = gl_ctx.pop_fun_def("main") {
        if let Err(err) = ty_check_global_fun_def(&mut gl_ctx, &mut nat_ctx, &mut main_fun, &arena)
        {
            err.emit(compil_unit.source);
            Err(ErrorReported)
        } else {
            gl_ctx.push_fun_checked_under_nats(main_fun, arena.alloc(vec![]));
            Ok(())
        }
    } else {
...
fn ty_check_expr<'a>(
    ...
    let ty = match &mut expr.expr {
        ...
        ExprKind::DepApp(fn_ident, gen_args) => Ty::new(TyKind::FnTy(
            arena.alloc(ty_check_dep_app(ctx, fn_ident, gen_args)?),
        )),
...
fn ty_check_sync<'a>(
    ...
    Ok(Ty::new(TyKind::Data(
        arena.alloc(DataTy::new(DataTyKind::Scalar(ScalarTy::Unit))),
    )))
}
...
fn ty_check_for<'a>(
    ...
    frame.append_idents_typed(vec![IdentTyped::new(
        ident.clone(),
        Ty::new(TyKind::Data(arena.alloc(DataTy::new(ident_dty)))),
        Mutability::Const,
        ctx.exec.clone(),
    )]);
...
fn ty_check_split<'a>(ctx: &mut ExprTyCtx, indep: &mut Split<'a>) -> TyResult<'a, Ty<'a>> {
    // exec::ty_check(
    //     ctx.kind_ctx,
    //     ctx.ty_ctx,
    //     ctx.ident_exec,
    ...
    for i in 0..indep.branch_bodies.len() {
        ...
        if indep.branch_bodies[i].ty.as_ref().unwrap().ty
            != TyKind::Data(arena.alloc(DataTy::new(DataTyKind::Scalar(ScalarTy::Unit))))
        {
            return Err(TyError::String(
...
fn ty_check_sched<'a>(ctx: &mut ExprTyCtx, sched: &'a mut Sched<'a>) -> TyResult<'a, Ty<'a>> {
    exec::ty_check(
        ctx.nat_ctx,
        ctx.ty_ctx,
        ctx.ident_exec,
...
fn ty_check_block<'a>(ctx: &mut ExprTyCtx, block: &'a mut Block<'a>) -> TyResult<'a, Ty<'a>> {
    ctx.ty_ctx.push_empty_frame();
...
fn collect_valid_loans<'a>(ty_ctx: &TyCtx, mut loans: HashSet<Loan<'a>>) -> HashSet<Loan<'a>> {
...
fn check_mutable<'a>(ty_ctx: &TyCtx, pl: &'a Place<'a>) -> TyResult<'a, ()> {
...
fn ty_check_binary_op<'a>(
    ...
    let ret_dty = match bin_op {
        ...
        | BinOp::Neq => Ty::new(TyKind::Data(
            arena.alloc(DataTy::new(DataTyKind::Scalar(ScalarTy::Bool))),
        )),
    };
    match bin_op {
...
fn ty_check_app<'a>(
    ...
    let ret_dty_placeholder = Ty::new(TyKind::Data(
        arena.alloc(DataTy::new(utils::fresh_ident("ret_ty", DataTyKind::Ident))),
    ));
...
fn apply_exec_checked<'a>(fn_ty: &'a mut FnTy<'a>, exec: &'a ExecExpr<'a>) -> TyResult<'a, ()> {
    // TODO reintroduce
    // exec::ty_check(
    //     ctx.kind_ctx,
    //     ctx.ty_ctx,
    ...
    let shared_mem_idents_ty = app_kernel
        ...
        .map(|dty| {
            IdentTyped::new(
                ...
                Ty::new(TyKind::Data(arena.alloc(DataTy::new(DataTyKind::At(
...
fn exec_distrib_over_blocks<'a>(exec_expr: &'a ExecExpr<'a>) -> ExecExpr<'a> {
...
fn ty_check_tuple<'a>(ctx: &mut ExprTyCtx, elems: &'a mut [Expr<'a>]) -> TyResult<'a, Ty<'a>> {
    ...
    let elem_tys: TyResult<Vec<_>> = elems
        ...
        .collect();
    Ok(Ty::new(TyKind::Data(
        arena.alloc(DataTy::new(DataTyKind::Tuple(elem_tys?))),
    )))
}

fn ty_check_proj<'a>(ctx: &mut ExprTyCtx, e: &'a mut Expr<'a>, i: usize) -> TyResult<'a, Ty<'a>> {
...
fn ty_check_array<'a>(ctx: &mut ExprTyCtx, elems: &'a mut Vec<Expr<'a>>) -> TyResult<'a, Ty<'a>> {
...
fn ty_check_literal<'a>(l: &mut Lit) -> Ty<'a> {
    ...
    Ty::new(TyKind::Data(
        arena.alloc(DataTy::new(DataTyKind::Scalar(scalar_data))),
    ))
}
    ...
    match (pattern, &pattern_dty.dty) {
        ...
        (Pattern::Tuple(patterns), DataTyKind::Tuple(elem_tys)) => {
            for (p, tty) in patterns.iter().zip(elem_tys) {
                infer_pattern_ident_tys(ctx, p, &Ty::new(TyKind::Data(arena.alloc(tty.clone()))))?;
            }
            Ok(())
        }
...
fn ty_check_let_uninit<'a>(
    ...
    if let TyKind::Data(dty) = &ty.ty {
        ...
        let ident_with_ty = IdentTyped::new(
            ident.clone(),
            Ty::new(TyKind::Data(
                arena.alloc(DataTy::new(DataTyKind::Dead(dty.clone()))),
            )),
            Mutability::Mut,
            exec_expr,
        );
...
fn ty_check_seq<'a>(ctx: &mut ExprTyCtx, es: &'a mut [Expr<'a>]) -> TyResult<'a, Ty<'a>> {
    for e in &mut *es {
        ty_check_expr(ctx, e)?;
...
fn ty_check_place<'a>(ctx: &mut ExprTyCtx, pl_expr: &'a mut PlaceExpr<'a>) -> TyResult<'a, Ty<'a>> {
    pl_expr::ty_check(&PlExprTyCtx::new(ctx, Ownership::Uniq), pl_expr)?;
...
fn ty_check_borrow<'a>(
    ...
    let res_dty = DataTy::new(DataTyKind::Ref(arena.alloc(RefDty::new(
...
pub fn callable_in<'a>(callee_exec_ty: &'a ExecTy<'a>, caller_exec_ty: &'a ExecTy<'a>) -> bool {
...
fn legal_exec_under_current<'a>(ctx: &ExprTyCtx, exec: &'a ExecExpr<'a>) -> TyResult<'a, ()> {
