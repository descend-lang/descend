...

    {
        ...
        auto tmp_scan_block = 0u;
        {
            for (std::size_t k = 1; (k <= 16); k = (k * 2u)) {
                tmp_shfl_res =
                        descend::shfl_up(l_thread_out_len_scan, (descend::i32)(k));
                if (((descend::to_warps().thread_rank() - 0) < k)) {
                } else {
        ...
        if (((descend::to_warps().meta_group_rank() - 0) < 7)) {
            {
                const auto s_scan_arr_item =
                        (&(&(*s_scan_arr))[(descend::to_warps().meta_group_rank() - 0)]);
                if (((descend::to_warps().thread_rank() - 0) < 31)) {
                } else {
        ...
        __syncthreads();
        if (((descend::to_warps().meta_group_rank() - 0) < 1)) {
            {
                if (((descend::to_warps().thread_rank() - 0) < 7)) {
                    {
                        const auto s_scan_arr_item =
                                (&(&(*s_scan_arr))[(descend::to_warps().thread_rank() - 0)]);
                ...
                for (std::size_t k = 1; (k <= 4); k = (k * 2u)) {
                    tmp_shfl_res = descend::shfl_up(tmp_scan_block, (descend::i32)(k));
                    if (((descend::to_warps().thread_rank() - 0) < k)) {
                    } else {
        ...
        if (((descend::to_warps().meta_group_rank() - 0) < 1)) {
            ...
            {
                const auto s_scan_arr_item =
                        (&(&(*s_scan_arr))[(descend::to_warps().meta_group_rank() - 1)]);
                {
        ...
        __syncthreads();
        {
            ...
            descend::atomic_store(descend::atomic_ref<descend::u32>((*s_block_out_item)),
                                  0u);
        }
