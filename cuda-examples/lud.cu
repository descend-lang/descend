...
__global__ auto lud_internal(descend::f32 *const m3) -> void {
  ...
  {{{{const auto peri_row_shared_block = (&(*peri_row[(
          ((((((blockIdx.y - 0) * (((matrix_dim / tile_size) - it) - 1)) +
              (blockIdx.x - 0)) *
             tile_size) +
            (threadIdx.y - 0)) *
           tile_size) +
          (threadIdx.x - 0))]));
  const auto peri_col_shared_block = (&(*peri_col[(
      ((((((blockIdx.y - 0) * (((matrix_dim / tile_size) - it) - 1)) +
          (blockIdx.x - 0)) *
         tile_size) +
        (threadIdx.y - 0)) *
