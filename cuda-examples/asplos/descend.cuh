...
public:
    ...
    // Compute runtime as time from beginning of first kernel execution to end of last
    auto full_runtime_in_ms() const -> float {
        float time = 0;
        CHECK_CUDA_ERR( cudaEventElapsedTime(&time, timings_.front().begin(), timings_.back().end()) );
        return time;
    }
...
__device__ __forceinline__ descend::u32 shfl_sync(descend::u32 warp_offset, descend::u32 leader) {
  return __shfl_sync(FULL_MASK, warp_offset, leader);
}

__device__ __forceinline__ descend::u32 ballot_sync(bool cond) {
  return __ballot_sync(FULL_MASK, cond);
}

__device__ u32 get_warp_id() {
  return threadIdx.x/32;
}
}; // end namespace descend
